import "./css/miniProfile.css";

const user_info = {
  id: 1,
  first_name: "Герман",
  login: "german",
  avatar_link: ""
}

function goto_profile() {}

const MiniProfile = () => {


  return (
    <div className={"mini-profile"} onClick={goto_profile()}>
      <div className="profile-info">
        <img src={user_info.avatar_link} alt=""/>
        <div className={"name-info"}>
          <p className="firstname">{user_info.first_name}</p>
          <p className="login">{user_info.login}</p>
        </div>
      </div>
      <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
        <path fillRule="evenodd" clipRule="evenodd"
              d="M17 12C17 13.3807 18.1193 14.5 19.5 14.5C20.8807 14.5 22 13.3807 22 12C22 10.6193 20.8807 9.5 19.5 9.5C18.1193 9.5 17 10.6193 17 12Z" fill="currentColor"/>
        <path fillRule="evenodd" clipRule="evenodd"
              d="M9.5 12C9.5 13.3807 10.6193 14.5 12 14.5C13.3807 14.5 14.5 13.3807 14.5 12C14.5 10.6193 13.3807 9.5 12 9.5C10.6193 9.5 9.5 10.6193 9.5 12Z" fill="currentColor"/>
        <path fillRule="evenodd" clipRule="evenodd" d="M2 12C2 13.3807 3.11929 14.5 4.5 14.5C5.88071 14.5 7 13.3807 7 12C7 10.6193 5.88071 9.5 4.5 9.5C3.11929 9.5 2 10.6193 2 12Z"
              fill="currentColor"/>
      </svg>
    </div>
  )
}

export default MiniProfile;